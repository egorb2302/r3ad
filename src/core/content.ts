...
export interface ContentDoc {
  ...
  toc: TocEntry[];
  /**
   * Обложка как data-URI, если издание её объявило.
   *
   * На страницу она не попадает — том открывается с первой полосы текста, а не
   * с картинки. Нужна ради одного: из её доминанты берётся цвет корешка на
   * полке (SPEC §7.3), поэтому и хранится уменьшенной.
   */
  cover?: string;
  charCount: number;
  imageCount: number;
