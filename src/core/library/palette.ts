...
function paletteFromHsl(base: Hsl): SpinePalette {
  const cloth = toCloth(base);

  // Накладка отбивается от поля светлотой, а не другим тоном: у переплёта это
  // тот же материал, только другой прокрас.
  const panel: Hsl = {
    h: cloth.h,
  ...
  // Золото на тёмном поле, блинт (тиснение без краски) — на светлом.
  const foil: Hsl =
    cloth.l < 0.34
      ? { h: 44, s: 0.52, l: 0.68 }
      : { h: cloth.h, s: cloth.s * 0.5, l: Math.max(0.1, cloth.l - 0.26) };

  // Каптал берут контрастным к крышке — обычно из дополнительного тона.
  const head: Hsl = { h: (cloth.h + 170) % 360, s: 0.42, l: 0.56 };

  return {
    cloth: hslToHex(cloth),
...
/** Палитра по названию и автору. Детерминирована. */
export function paletteFor(seed: string): SpinePalette {
  const h = hashString(seed);
  return paletteFromHsl({
    h: CLOTH_HUES[h % CLOTH_HUES.length],
...
/** Палитра по доминанте обложки. */
export function paletteFromColor(hsl: Hsl): SpinePalette {
