...
export type VolumeSource =
  ...
  /**
   * Корешок приехал, текст — нет.
   *
   * Дефолтный объём шеринга — «только внешний вид» (SPEC §11.2): полка
   * выгружается корешками, содержимое книг остаётся дома. Такой том на чужой
   * полке стоит настоящий, нужной толщины и цвета, и не открывается — вместо
   * текста в нём одна страница, которая честно говорит, почему его тут нет.
   * Это не деградация, а то, ради чего дефолт такой и выбран (§18).
   */
